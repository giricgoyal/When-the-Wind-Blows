// Generated by EclipseP5Exporter: Tue Nov 27 18:32:10 CST 2012
/**
 * 
 */
...
@SuppressWarnings("serial")
	...
	void setCurrentData(ArrayList<DataPos> current, ArrayList<DataPos> day, int bHalf, int eHalf, String keyword) {
		current.clear();
	...
	void setCurrentData_forKeywords(ArrayList<DataPos> current, ArrayList<DataPos> day, int bHalf, int eHalf, String keyword) {
		current.clear();
		System.out.println("start updating current data");
		for (DataPos data : day) {
			int half = data.getHour()*2 + data.getMin()/30;
	...
	void setMarkerPos(ArrayList<DataPos> dataPos, ArrayList<AbstractMarker> markers, MarkerType type) {
		markers.clear();
	...
	private ArrayList<WordCountPair> getWordCountPair(String[] words) {
		...
		Collections.sort(entry, new Comparator<WordCountPair>() {
		    int compare(WordCountPair a, WordCountPair b) {
	...
	boolean isIn(float mx, float my, float bx, float by, float bw,
			float bh) {
		return (bx <= mx && mx <= bx + bw && by <= my && my <= by + bh);
	}
	
	boolean isInCenter(float _x, float _y, float x, float y, float w, float h) {
	...
	void myPressed(int id, float mx, float my) {
	...
	void myDragged(int id, float mx, float my) {
		if (isTouchingMap) {
	...
	void myReleased(int id, float mx, float my) {
		//touchList.remove(id);
	...
	void myClicked(int id, float mx, float my) {
	}
	...
	void touchDown(int ID, float xPos, float yPos, float xWidth,
			float yWidth) {
	...
	void touchMove(int ID, float xPos, float yPos, float xWidth,
			float yWidth) {
		myDragged(ID, xPos, yPos);
	}

	void touchUp(int ID, float xPos, float yPos, float xWidth,
			float yWidth) {
		touchList.remove(ID);
